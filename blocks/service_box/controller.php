...
class ServiceBoxBlockController extends BlockController {
	
	protected $btName = 'Service Box';
	protected $btDescription = 'Service Box dropdown';
	protected $btTable = 'btDCServiceBox';
	...
	public function add() {
		...
		$field_6_default_content = '<p> Our service </p>';
		$this->set('field_6_wysiwyg_content', $field_6_default_content);
	}
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ServiceBoxBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ServiceBoxBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('ServiceBoxBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('ServiceBoxBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ServiceBoxBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ServiceBoxBlockController', 'replaceDownloadFileID'),
			$text);
